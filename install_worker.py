...
def main():
    ...
    # Setup terminal
    run_remote_command(ssh, "sudo apt-get update")
    ...
    # Init K8s
    run_remote_command(["kubeadm", "reset", "-f"])
    run_remote_command(["systemctl", "daemon-reload"])
    run_remote_command(["service", "kubelet", "start"])

    print("\nEXECUTE ON MASTER: kubeadm token create --print-join-command --ttl 0")
    print("THEN RUN THE OUTPUT AS COMMAND HERE TO ADD AS WORKER")
    print()
